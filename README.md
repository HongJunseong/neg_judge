...
교통사고 발생 시 블랙박스 영상을 활용해 과실 비율을 자동으로 산정하는 시스템은 보험 심사와 분쟁 조정을 빠르고 객관적으로 수행하는 데 도움을 줄 수 있다.
   
본 프로젝트에서는 블랙박스 영상으로부터 프레임을 추출하고, Temporal Segment Network(TSN) 방식을 적용해 각 구간의 대표 프레임을 선정한 뒤 이를 ResNet34 백본으로 학습하여 사고 발생 장소를 분류하였다. 이어지는 단계에서는 추출된 대표 프레임 시퀀스와 사고 장소 예측 결과를 함께 입력으로 사용하여, SlowFast 네트워크를 통해 사고 특징과 차량 A·B의 진행 방향을 동시에 예측하였다. 마지막 단계에서는 이전 단계의 예측값을 결합한 벡터를 MLP 분류기에 입력하여 과실 비율을 구간별로 판단하였으며, Optuna를 활용한 하이퍼 파라미터 최적화를 통해 안정적인 학습 성능을 확보하였다. 제안된 시스템은 블랙박스 영상만으로 과실 비율을 자동 추정할 수 있는 가능성을 제시하며, 향후 교통사고 처리의 객관화 및 효율성 향상에 기여할 것으로 기대된다.
